...
fn struct_attrs(attr: &Attribute) -> StructAttrs {
    attr.parse_args::<StructAttrs>()
        .expect("failed to parse attributes")
}

fn get_attr<'field>(attrs: &'field [Attribute], name: &str) -> Option<&'field Attribute> {
    attrs
        .iter()
...
fn name(struct_attr: &StructAttrs, field: &Field) -> Option<String> {
